...
(async() => {
    ...
    await searchPage.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/88.0.4298.0 Safari/537.36');
    ...
    await snapPage.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/88.0.4298.0 Safari/537.36');
    ...
    await zoomPage.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/88.0.4298.0 Safari/537.36');
    ...
    await equityPage.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/88.0.4298.0 Safari/537.36');
    await equityPage.setViewport({ width: 1700, height: 3000 });
    await equityPage.goto('https://app.flowalgo.com/snapshot/equity/?period=today');

    /* Setup page for io */
    const ioPage = await browser.newPage();
    await ioPage.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/88.0.4298.0 Safari/537.36');
    ...
    await page.exposeFunction('newFlow', flow => {
        ...
        .setColor(flow.isUnusual ? '#950FF5' : flow.isGolden ? '#F5CB0F' : flow.isCall ? '#33CC6B' : '#CC4A33')
        .setTitle(`${flow.ticker} ${flow.isCall ? 'CALLS' : 'PUTS'} ${flow.isUnusual ? '(UNUSUAL)' : flow.isGolden ? '(GOLDEN SIGNAL)' : ''}`)
        .setDescription(`Score: **[${getProgressBar(flow.algoScore)}]** *(${flow.algoScore}/100)*`)
        .addFields(
        ...
        guilds.forEach(guild => {
            const channel = bot.channels.cache.get(flow.isUnusual ? guild.unusual : flow.isGolden ? guild.golden : guild.flow);
            if(!channel) return;
            channel.send(flow.isUnusual ? guild.unusual_ping : flow.isGolden ? guild.golden_ping : guild.flow_ping , embed).catch(() => {});
        });
    });
    ...
    await page.exposeFunction('newDarkflow', darkflow => {
        ...
        const embed = new MessageEmbed()
        .setColor(darkflow.greenEquity ? '#4be352' : darkflow.darkprint ? '#000000' : '#9ea2a8')
        .setTitle(`${darkflow.ticker} ${darkflow.greenEquity ? 'Green Equity' : darkflow.darkprint ? 'Darkpool Prints' : 'Equity Blocks'}`)
        .addFields(
        ...
        guilds.forEach(guild => {
            const channel = bot.channels.cache.get(darkflow.greenEquity ? guild.greenequity : darkflow.darkprint ? guild.darkprint : guild.equity);
            if(!channel) return;
            channel.send(darkflow.greenEquity ? guild.greenequity_ping : darkflow.darkprint ? guild.darkprint_ping : guild.equity_ping, embed).catch(() => {});
        })
    })
    ...
    await page.exposeFunction('newAlphaSig', alphaSig => {
        ...
        guilds.forEach(guild => {
            ...
            if(!channel) return;
            channel.send(guild.alphaai_ping, embed).catch(() => {});
        })
    })
    ...
    await page.evaluate(() => {
        ...
        const flowObserver = new MutationObserver(function(){
            const flow = document.querySelector('.optionflow-component>*>.data-body > div');

            let details = flow.querySelector('.details>span').innerText;
            details = details.split('@')[0].trim() + ' @ $' + details.split('@')[1].trim();
        ...
        flowObserver.observe(document.querySelector('.optionflow-component>*>.data-body'), { childList: true, subtree: true })
        ...
        const darkflowObserver = new MutationObserver(function(){
            const darkflow = document.querySelector('.darkflow-component>*>.data-body > div');
            ...
            const data = {
                ...
                greenEquity: parseInt(darkflow.querySelector('.notional').innerText.replace(/[^0-9]/g, '')) >= 50
            };
        ...
        darkflowObserver.observe(document.querySelector('.darkflow-component>*>.data-body'), { childList: true, subtree: true });
        ...
        alphaSigObserver.observe(document.querySelector('#fa_aai'), { childList: true, subtree: true });
    })
})();
